//CREDITOS  𝐾𝐴𝑁𝑇 𝐿𝑆 BOT PELA BASE 
...
async function starts() {
    ...
    •            @${num.split('@')[0]}was E j e c t e d
                      1 impostor restante   。　.
    　 　　。　　 　　　　ﾟ　　　.　      　　　.
    ,　　VAI PELAS SOMBRAS @${num.split('@')[0]}  FUCK😂👋`
		...
		try {
			...
			mess = {
      		...
      		mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD PRIVATE BOT*',
					...
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de 𝐾𝐴𝑁𝑇 𝐿𝑆. Registre-se para fazer amizade com o bot 𝐾𝐴𝑁𝑇 𝐿𝑆 por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar seunome|17\n\n──「 𝐾𝐴𝑁𝑇 𝐿𝑆 」──`,
			...
			const ownerNumber = ["557182507697@s.whatsapp.net"] // coloque seu numero
			const mod = [ownerNumber,"557182507697@s.whatsapp.net"]//seu numero
			const adminbotnumber = ["557182507697@s.whatsapp.net"]//seu numero
			const frendsowner = ["557182507697@s.whatsapp.net"]//seu numero
			const premium = ["557182507697@s.whatsapp.net","553299114579@s.whatsapp.net",] 
               ...
               break
				...
				case 'bc3':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						reply('Transmissao do 𝐾𝐴𝑁𝑇 𝐿𝑆')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ *TRANSMISSÃO 𝐾𝐴𝑁𝑇 𝐿𝑆* ]\n\n${body.slice(4)}`)
						}
						reply('Transmissão enviada')
					}
					break
...
case 'figu':
				...
				case 'stiker':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(`Envie fotos ou videos de no maximo 9s com legendas *.f* ou marque uma imagem que já foi enviada`)
					}
					break
				case 'attp' :
				case 'sttp' :
...
case 'play':   
	        if (!isUser) return reply(mess.only.daftarB)
                reply(mess.wait)
                play = body.slice(5)
                anu = await fetchJson(`https://api.zeks.xyz/api/ytplaymp3?apikey=${play}&apikey=${ZeksApi}`)
               if (anu.error) return reply(anu.error)
...
case 'antilink':
				...
				//INFO BOT
case 'owner':
                    client.sendMessage(from, {displayname: "KANT", vcard: vcard}, MessageType.contact, { quoted: mek})
                    client.sendMessage(from, 'Ctt do meu  𝐾𝐴𝑁𝑇 𝐿𝑆 ai, pfv n flode o chat',MessageType.text, { quoted: mek} )
                    break
case 'speed':
                case 'ping':
...
break
case 'arrombado':
tujuh = fs.readFileSync('./assets/chupamnhpika.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'a':
tujuh = fs.readFileSync('./assets/a.mp3');
...
break
                ...
                case 'leveling':
                   ...
                   case 'ddd':
                    if (!isPremium) return reply(mess.only.premium)
                    if (args.length < 1) return reply(`*Qual DDD Você Quer Consultar?*`)
                    var dataC = await fetchJson(`https://lucasssmodswar.herokuapp.com/apis/dddsbrasileiros_apikey=lucasss/${args[0]}.json`, {method: 'get'})
          ...
          reply('Espere..')
                                       ...
                                       if (args[0].startsWith('557182507697')) return reply('Falha ao ligar para o número do proprietário')
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/5571982507697/*╰──────*「 * kant * 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO kant BOT_*\n*_Ketik #daftarvip*' })
					break
case 'daftarvip': 
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:*  𝐾𝐴𝑁𝑇 𝐿𝑆 /n*YOUTUBE:* https://youtube.com/c/\n*WPP:* wa.me/5571982507697\n*INSTA:* @vitornnb\n\n\nEspero que tenham gostado do bot 🐊🚩️'})
					break
				//_IMAGENS
				case 'boanoite':
					memein = await kagApi.memeindo()
